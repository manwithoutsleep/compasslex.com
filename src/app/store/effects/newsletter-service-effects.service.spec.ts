...
describe('NewsletterServiceEffects', () => {
    ...
    describe('getNewsletterListAction', () => {
        it(
            'should fire off a getNewsletterListSuccessAction when service returns success',
            marbles((m) => {
                const expectedResponse = { newsletterList } as NewsletterData;

                mockNewsletterService.getNewsletterList = (): Observable<NewsletterData> =>
                    of(expectedResponse);
        ...
        it(
            'should fire off a getNewsletterListFailureAction when an HTTP error occurs',
            marbles((m) => {
                mockNewsletterService.getNewsletterList = (): Observable<NewsletterData> =>
